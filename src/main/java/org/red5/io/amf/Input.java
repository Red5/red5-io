...
public class Input extends BaseInput implements org.red5.io.object.Input {
	...
	protected static Map<String, String> classAliases = new HashMap<String, String>(
			3) {
		{
			put("DSA",
	...
	public Input(IoBuffer buf) {
		...
		if (log.isTraceEnabled()) {
			log.trace(
					"Input: {}",
					Hex.encodeHexString(Arrays.copyOfRange(buf.array(),
	...
	/**
	 * Reads a Number. In ActionScript 1 and 2 Number type represents all
	...
	public Number readNumber() {
		...
		if (log.isDebugEnabled()) {
			log.debug(
					"Remaining not big enough for number - offset: {} limit: {} {}",
					buf.position(), buf.limit(),
	...
	public Object readArray(Type target) {
		...
		// To conform to the Input API, we should convert the output into an
		// Array if the Type asks us to.
		...
		storeReference(result); // reference should be stored before reading of
								// objects to get correct refIds
		for (int i = 0; i < count; i++) {
			resultCollection.add(Deserializer.deserialize(this, Object.class));
		}
		if (collection.isArray()) {
			ArrayUtils.fillArray(collection.getComponentType(), result,
					resultCollection);
		}
		return result;
	}
			...
			if (className != null) {
				...
				if (className.equals("RecordSet")) {
					...
					if (instance != null) {
						...
						log.debug("Forced to use simple object for class {}",
								className);
						result = readSimpleObject();
					}
				}
	...
	protected Type getPropertyType(Object instance, String propertyName) {
		try {
			...
			try {
				...
				PropertyUtilsBean propertyUtils = beanUtilsBean
						.getPropertyUtils();
				PropertyDescriptor propertyDescriptor = propertyUtils
						.getPropertyDescriptor(instance, propertyName);
